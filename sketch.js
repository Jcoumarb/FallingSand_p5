...
function draw (p) { 
	...
	for(let i = 0; i < cols; i++) {
		for(let j = 0; j < rows; j++) {
			...
			if(state.value === 1) {
				let below = grid[i] && grid[i][j + 1] ? grid[i][j + 1] : null;
				let dir = p.random([-1, 1]);
				let belowA, belowB;
				if(i + dir >= 0 && i + dir <= cols - 1) belowA = grid[i + dir][j + 1];
				if(i - dir >= 0 && i - dir <= cols - 1) belowB = grid[i - dir][j + 1];
				...
				if (!below || below.value === 0) {
					if (j + 1 < rows) {
						nextGrid[i][j + 1] = { value: 1, color: state.color };
						nextGrid[i][j] = { value: 0, color: [0, 0, 0] };
					}
				} else if (!belowA || belowA.value === 0) {
					if (j + 1 < rows && i + dir >= 0 && i + dir < cols) {
						nextGrid[i + dir][j + 1] = { value: 1, color: state.color };
						nextGrid[i][j] = { value: 0, color: [0, 0, 0] };
					}
